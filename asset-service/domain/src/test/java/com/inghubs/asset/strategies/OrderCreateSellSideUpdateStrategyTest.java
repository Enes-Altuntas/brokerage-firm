...
class OrderCreateSellSideUpdateStrategyTest {

  @InjectMocks
  private OrderCreateSellSideUpdateStrategy strategy;
  ...
  void updateAsset_shouldValidateOrder_whenAssetIsValid() {
    ...
    verify(outboxPort).createOrderOutboxEntity(OrderCreateSellSideUpdateStrategy.ORDER_VALIDATED, order);
  ...
  void updateAsset_shouldRejectOrder_whenAssetIsInvalid() {
    ...
    // Then
    verify(outboxPort).createOrderOutboxEntity(OrderCreateSellSideUpdateStrategy.ORDER_REJECTED, order);
    verify(inboxPort).createInboxEntity(command);
  }
}