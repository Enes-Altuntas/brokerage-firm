...
class OrderCancelBuySideUpdateStrategyTest {

  @InjectMocks
  private OrderCancelBuySideUpdateStrategy strategy;
  ...
  void updateAsset_shouldConfirmCancel_whenAssetIsValid() {
    ...
    verify(outboxPort).createOrderOutboxEntity(OrderCancelBuySideUpdateStrategy.ORDER_CANCEL_CONFIRMED, order);
  ...
  void updateAsset_shouldRejectCancel_whenAssetIsInvalid() {
    ...
    // Then
    verify(outboxPort).createOrderOutboxEntity(OrderCancelBuySideUpdateStrategy.ORDER_CANCEL_REJECTED, order);
    verify(inboxPort).createInboxEntity(command);
  }
}